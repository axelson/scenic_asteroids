defmodule Play.Scene.Primitives do
  @moduledoc """
  Sample scene.
  """
  ...
  alias Play.Component.Notes

  @bird_hash "0DMsqJaAU2OyRdd9Hp3WWJoO3WE"
  @bird_path :code.priv_dir(:play)
         ...
         |> group(
           fn g ->
             ...
             |> group(
               fn g ->
                 g
                 |> text("Hello", translate: {0, 40}, id: :hello_text)
                 |> text("Worlds2", translate: {90, 40}, fill: :yellow)
                 |> text("Blurry text", translate: {0, 80}, font_blur: 2)
                 |> text("Shadow", translate: {82, 102}, font_blur: 2, fill: :light_grey)
                 |> text("Shadow", translate: {80, 100})
               end,
               font_size: 40,
               t: {130, 240}
             )
  ...
  def handle_info({:add_new}, state) do
    IO.puts("in handle_info add new!")
    {:noreply, state}
  end

  def handle_info(mesg, state) do
    IO.puts("handle_info got other mesg: #{inspect(mesg)}")
    {:noreply, state}
  end

  def handle_call(mesg, _from, graph) do
    IO.puts("got other mesg: #{inspect(mesg)}")
    graph = add_new(graph)
    {:reply, graph, graph}
  end

  def add_new(graph) do
    # graph =
    #   Scenic.Graph.build(font: :roboto, font_size: 24)
    #   |> Scenic.Primitives.text("Blah blah")
    graph
    |> Graph.modify(:hello_text, &text(&1, "ive ben modded!", translate: {0, 60}))
    |> push_graph()
  end
end
