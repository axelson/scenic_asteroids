defmodule Play.Scene.Transforms do
  use Scenic.Scene
  alias Scenic.Graph
  ...
  |> group( fn(g) ->
    ...
    end, translate: {60, 20}, text_align: :right)
    |> group( fn(g) ->
      g
  ...
  |> group( fn(g) ->
    ...
    |> quad({{0,20},{30,0},{36,26},{25,40}}, id: :quad,
    fill: {:linear, {0,0,40,40,:yellow,:purple}}, stroke: {2, :khaki},
    translate: {140, 0}, scale: 1.4) # pin: {400,310}
    |> slider({{-1.5708,1.5708},0}, id: :rotate_quad, translate: {0,50}, width: 200)
  end, translate: {@start_x, @start_y}, pin: {100, 25}, id: :ui_group)
  ...
  #--------------------------------------------------------
  def filter_event( {:value_changed, :pos_x, x}, _, %{
    graph: graph,
    y: y
  } = state ) do
    graph = graph
  ...
  #--------------------------------------------------------
  def filter_event( {:value_changed, :pos_y, y}, _, %{
    graph: graph,
    x: x
  } = state ) do
    graph = graph
